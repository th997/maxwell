...
public class DorisLogic {
	...
	public DorisLogic(JdbcProducer jdbcProducer) throws IOException {
		...
		headers = om.readValue(properties.getProperty("doris.headers", "{}"), new TypeReference<Map<String, Map<String, String>>>() {
		});
		this.httpClient = HttpClients.custom().setDefaultRequestConfig(RequestConfig.custom().setConnectTimeout(connectionTimeout).setSocketTimeout(readTimeout).build()).setRedirectStrategy(new DefaultRedirectStrategy() {
			@Override
	...
	public void streamLoad(String schema, String table, List<Map<String, Object>> list) {
		...
		try {
			...
			try (CloseableHttpResponse response = httpClient.execute(httpPut)) {
				...
				if (statusCode != 200) {
					throw new RuntimeException("streamLoad failed,database=" + schema + ",table=" + table + ",statusCode=" + statusCode + ",headers=" + om.writeValueAsString(httpPut.getAllHeaders()) + ",res=" + res);
				} else {
